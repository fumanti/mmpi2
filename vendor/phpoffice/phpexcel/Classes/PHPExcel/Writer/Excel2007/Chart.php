...
class PHPExcel_Writer_Excel2007_Chart extends
  ...
  private function _writePlotArea(PHPExcel_Chart_PlotArea $plotArea,
    ...
    if (($chartType !== PHPExcel_Chart_DataSeries::TYPE_PIECHART) &&
      ...
      // <c:spPr><a:ln w="12700"><a:solidFill><a:srgbClr val="FF0000"/></a:solidFill></a:ln></c:spPr></c:plotArea><c:plotVisOnly val="1"/>
  ...
  private function _writeCatAx($objWriter, PHPExcel_Chart_PlotArea $plotArea, $xAxisLabel, $groupType, $id1, $id2, $isMultiLevelSeries, $xAxis, $yAxis) {
    ...
    $objWriter->writeAttribute('val', 'none'); // $yAxis->getAxisOptionsProperty('major_tick_mark'));
    $objWriter->endElement();

    $objWriter->startElement('c:minorTickMark');
    $objWriter->writeAttribute('val', 'out'); //$yAxis->getAxisOptionsProperty('minor_tick_mark'));
    $objWriter->endElement();
  ...
  private function _writeValAx($objWriter, PHPExcel_Chart_PlotArea $plotArea, $yAxisLabel, $groupType, $id1, $id2, $isMultiLevelSeries, $xAxis, $yAxis, $majorGridlines, $minorGridlines) {
    ...
    if (!is_null($xAxis->getAxisOptionsProperty('maximum'))) {
      ...
      $objWriter->writeAttribute('val', 120); //$xAxis->getAxisOptionsProperty('maximum'));
      $objWriter->endElement();
    }

    if (!is_null($xAxis->getAxisOptionsProperty('minimum'))) {
      $objWriter->startElement('c:min');
      $objWriter->writeAttribute('val', 30); //$xAxis->getAxisOptionsProperty('minimum'));
      $objWriter->endElement();
    }
    ...
    $objWriter->startElement('c:majorTickMark');
    $objWriter->writeAttribute('val', 'out'); // $xAxis->getAxisOptionsProperty('major_tick_mark'));
    $objWriter->endElement();

    $objWriter->startElement('c:minorTickMark');
    $objWriter->writeAttribute('val', 'in'); //$xAxis->getAxisOptionsProperty('minor_tick_mark'));
    $objWriter->endElement();
    ...
    if ($id1 > 0) {
      ...
      if (!is_null($xAxis->getAxisOptionsProperty('major_unit'))) {
        ...
        $objWriter->writeAttribute('val', 5); //$xAxis->getAxisOptionsProperty('major_unit'));
        $objWriter->endElement();
      }

      if (!is_null($xAxis->getAxisOptionsProperty('minor_unit'))) {
        $objWriter->startElement('c:minorUnit');
        $objWriter->writeAttribute('val', 1); //$xAxis->getAxisOptionsProperty('minor_unit'));
        $objWriter->endElement();
      }
